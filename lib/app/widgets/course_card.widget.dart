              ...
              Center(
                child: Consumer(
                  builder: (context, ref, child) {
                    final isInWatchlist = ref.watch(watchlistVM.notifier).isInWatchlist(id);
                    return IconButton(
                      onPressed: () {
                        if(isInWatchlist){
                          ref.read(watchlistVM.notifier).removeFromWatchlist(id);
                        } else {
                          ref.read(watchlistVM.notifier).addToWatchlist(id, Course(id: id, title: title, description: description, image: image));
                        }
                      },
